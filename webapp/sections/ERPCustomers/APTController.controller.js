  ...
  function (Controller, MessageBox) {
    ...
    return Controller.extend("Iffco.clap.sections.ERPCustomers.APTController", {
      onInit: function () {
        ...
        if (!this.GroupingKey) {
          this.GroupingKey = new sap.ui.xmlfragment(
            "Iffco.clap.fragments.GroupingKey",
            this
          );
          this.getView().addDependent(this.GroupingKey);
        }
        if (!this.SortKey) {
          this.SortKey = new sap.ui.xmlfragment(
            "Iffco.clap.fragments.SortKey",
            this
          );
          this.getView().addDependent(this.SortKey);
        }
        if (!this.HouseBank) {
          this.HouseBank = new sap.ui.xmlfragment(
            "Iffco.clap.fragments.HouseBank",
            this
          );
          this.getView().addDependent(this.HouseBank);
        }
        if (!this.PaymntMethdSuplmnt) {
          this.PaymntMethdSuplmnt = new sap.ui.xmlfragment(
            "Iffco.clap.fragments.PaymntMethdSuplmnt",
            this
          );
          this.getView().addDependent(this.PaymntMethdSuplmnt);
          this.PaymntMethdSuplmnt.setModel(
            this.getOwnerComponent().getModel("S4D")
          );
        }
        if (!this.ReleaseGrp) {
          this.ReleaseGrp = new sap.ui.xmlfragment(
            "Iffco.clap.fragments.ReleaseGroup",
            this
          );
          this.getView().addDependent(this.ReleaseGrp);
        }
        if (!this.PlanningGrp) {
          this.PlanningGrp = new sap.ui.xmlfragment(
            "Iffco.clap.fragments.PlanningGroup",
            this
          );
          this.getView().addDependent(this.PlanningGrp);
        }
        if (!this.ValueAdjustment) {
          this.ValueAdjustment = new sap.ui.xmlfragment(
            "Iffco.clap.fragments.ValueAdjustment",
            this
          );
          this.getView().addDependent(this.ValueAdjustment);
        }
        if (!this.HeadOffice) {
          this.HeadOffice = new sap.ui.xmlfragment(
            "Iffco.clap.fragments.HeadOffice",
            this
          );
          this.getView().addDependent(this.HeadOffice);
        }
        if (!this.CompCode) {
          this.CompCode = new sap.ui.xmlfragment(
            "Iffco.clap.fragments.CompCode",
            this
          );
          this.getView().addDependent(this.CompCode);
        }
        if (!this.ReconNum) {
          this.ReconNum = new sap.ui.xmlfragment(
            "Iffco.clap.fragments.Reconsalition",
            this
          );
          this.getView().addDependent(this.ReconNum);
        }
      },
      ...
      handleReconcilationNumber: function (evt) {
        this.ReconNumField = evt.getSource();
        let sValue = this.getView()
          .getModel("Customers")
          .getData().zcompany_code;
        if (sValue) {
          var oFilter1 = new sap.ui.model.Filter(
            "Companycode",
            "Contains",
          ...
          this.ReconNum.getBinding("items").filter([oFilter1]);
          this.ReconNum.open();
        } else {
          MessageBox.error("Please select Company code first");
        }
      },
      handleReconSubmit: function (evt) {
        var title = evt.getParameter("selectedItems")[0].getProperty("title");
        var desc = evt
          .getParameter("selectedItems")[0]
          .getProperty("description");
        this.ReconNumField.setValue(title + " - " + desc);
        this.ReconNum.close();
      },

      handleReconSearch: function (evt) {
        var sValue = evt.getParameter("value");
        var filters = [];
        if (sValue.length > 0) {
          var filter1 = new sap.ui.model.Filter({
          ...
          var sFilters = [filter1, filter2];
          filters.push(new sap.ui.model.Filter(sFilters, false));
          this.ReconNum.getBinding("items").filter(filters, false);
        } else {
          let sCompanyCode = this.getView()
            .getModel("Customers")
            .getData().zcompany_code;
          var oFilter1 = new sap.ui.model.Filter(
            "Companycode",
